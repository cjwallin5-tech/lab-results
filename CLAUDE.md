...
7. **Safe states render no matter what:** critical → urgent callout (FR-07); implausible →
...
- Testing: Vitest colocated `*.test.ts`; Playwright E2E. Golden suite 100% required in CI.
  E2E covers the full loop including one critical, one implausible, one unknown test.
