...
/**
 * Parse a printed value into a number, or null when it is not a plain decimal.
 * Thousands separators are tolerated (platelet counts print as "1,234");
 * comparator-prefixed strings like "<5" are treated as non-numeric, because the
...
export function unitsMatch(reportUnit: string | undefined, analyteUnit: string): boolean {
  ...
  return UNIT_EQUIVALENCES.some((group) => group.includes(report) && group.includes(analyte));
}
