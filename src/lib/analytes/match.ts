...
/**
 * Pure name matching, safe to run on the client (no filesystem). The server
 * loader and the client-side verify preview both match printed names through
 * this so their behaviour stays identical.
 */
...
export function matchAnalyteIn(
  entries: AnalyteEntry[],
  rawName: string,
): AnalyteEntry | undefined {
