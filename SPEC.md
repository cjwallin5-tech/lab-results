...
6. **Classify (code, not AI):** deterministic comparison of each value against the report's
   ...
   our curated critical thresholds. Implausible values are flagged, not classified.
7. **Draft (AI):** one call generates the patient page — the "Overall picture" synthesis and
...
| S3 | Provider: verify extraction + review/approve draft | New — needs Figma design |
...
- **FR-02** Upload: PDF up to ~10 pages; stored with patient name/email/DOB and a status:
  `uploaded → extracted → verified → drafted → approved → sent`.
...
- **FR-07** Critical result: urgent callout on the patient page advising prompt contact with
  the provider; rendered even in the AI draft, not removable by prompt. Critical fires when the
  lab's printed flags indicate it **or** a curated critical threshold is crossed (either suffices);
  if a report's unit can't be confidently matched to the dictionary's unit, the curated-threshold
  check is skipped and only the printed range/flags are used — never guessed.
...
3. **E2E (Playwright):** full loop upload → verify → approve → link → DOB → results page,
   including one critical, one implausible, and one unknown test.
4. **Gate tests:** no route renders unapproved explanation text; disclaimer present on every
   patient view.
